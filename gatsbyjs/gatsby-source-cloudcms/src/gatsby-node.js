...
const { introspectSchema, buildSchema, print, printSchema } = require(`graphql`);
...
exports.sourceNodes = ({actions, createNodeId, cache, store}, options, callback) => {
    console.log("yeet");
            ...
            this.readBranch(branchId).then(function() {
                ...
                branch.graphqlSchema(function(sdl) {
                    ...
                    const fetcher = async ({query: queryDocument, variables, operationName, context}) => {
                        ...
                        return Chain(branch).graphqlQuery(query, operationName, variables, function(response) {
                        //     var deepSearch = function(item) {
                    ...
                    const node = createSchemaNode({ id: nodeId });
                    createNode(node);
                    ...
                    // Schema object here is of type GraphQLSchema, but it is not recognized as such in mergeSchemas.js... 
                    addThirdPartySchema({ schema: schema });
